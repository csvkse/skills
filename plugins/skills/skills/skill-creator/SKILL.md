...
4. **Launch the viewer** with both qualitative outputs and quantitative data:
   ...
   ```
   This starts a local server and opens it in the browser. Stop it with `Ctrl+C` when done. To run it in the background instead, use your shell's mechanism (`&` / `nohup` on Unix, `start /B` on Windows).
...
3. Write to a temp file (e.g., `./eval_review_<skill-name>.html` or the OS temp dir) and open it in the browser (macOS `open`, Windows `start`, Linux `xdg-open`)
...
---
Claude.ai / Cowork 等环境的差异说明见 `references/platform-instructions.md`。
...
- `references/output-patterns.md` — output structure patterns for skills that produce files
- `references/workflows.md` — workflow patterns (sequential, branching) for complex skills
- `references/platform-instructions.md` — Claude.ai / Cowork environment differences
