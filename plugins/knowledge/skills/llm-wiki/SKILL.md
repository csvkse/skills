...
> NOTE: If the wiki doesn't exist at `~/.agents/wiki`, check `~/wiki` as fallback.
...
llm-wiki（结构化知识）与 knowledge（过程记忆）可双向联动。
...
- `library-tips/*.md` → `concepts/`（使用模式）
- `problem-analysis/*.md` → `entities/`（问题实体）
- `decisions/*.md` → `concepts/`（决策模式）
...
1. **library-tips** → `concepts/`（提取通用模式）
2. **problem-analysis** → `entities/`（提取问题实体）
3. **decisions** → `concepts/`（提取决策模式）
4. **sessions** → 不联动（过程记录）
